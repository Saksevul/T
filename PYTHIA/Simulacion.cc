...
int main(int argc, char* argv[]) {   Float_t PI=3.1415927;
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {    if (!pythia.next()) continue;
    ...
    for (size_t i=0; i<sortedJets.size(); i++){   Bool_t FlagE=false, FlagM=false; Float_t pTE=0.0, pTM=0.0; Int_t motherE, motherM;
      ...
      for (size_t j=0; j<sortedJets[i].constituents().size(); j++) {  Int_t EvIndex=sortedJets[i].constituents()[j].user_info<MyInfo>().Index(); // Event Index.


        if ( abs(event[EvIndex].id()) == 13 ) {  motherM=event[EvIndex].mother1();
          h_Muons_pt_ -> Fill(sortedJets[i].constituents()[j].pt());
          if ( pow2(event[EvIndex].xProd()) + pow2(event[EvIndex].yProd()) + pow2(event[EvIndex].zProd()) <= 0.25 ) {
            h_Muons__pT -> Fill(sortedJets[i].constituents()[j].pt());
            h_Muons__fX -> Fill(event[EvIndex].xProd()/10.0);
            h_Muons__fY -> Fill(event[EvIndex].yProd()/10.0);
            h_Muons__fZ -> Fill(event[EvIndex].zProd()/10.0);
            h__pTQuotient_Muon_ak5FastJet -> Fill(sortedJets[i].constituents()[j].pt()/sortedJets[i].pt());
            if ( abs(event[EvIndex].phi() - sortedJets[i].phi_std()) <= PI ) {
              h__D__Muon_ak5FastJet -> Fill(sqrt( pow2(event[EvIndex].eta()-sortedJets[i].eta()) + pow2(event[EvIndex].phi()-sortedJets[i].phi_std()) ));
            } else {
              h__D__Muon_ak5FastJet -> Fill(sqrt( pow2(event[EvIndex].eta()-sortedJets[i].eta()) + pow2(2*PI-abs(event[EvIndex].phi()-sortedJets[i].phi_std())) ));
            }
          ...
          // while ( abs(event[motherM].id()) >= 6 ) motherM=event[motherM].mother1();
          // printf(" Madre final de este Muon = %i.\n", event[motherM].id());
        }

        if ( abs(event[EvIndex].id()) == 11 ) {  motherE=event[EvIndex].mother1();
          h_Electrons_pt_ -> Fill(sortedJets[i].constituents()[j].pt());
          if ( pow2(event[EvIndex].xProd()) + pow2(event[EvIndex].yProd()) + pow2(event[EvIndex].zProd()) <= 0.25 ) {
            h_Electrons__pT -> Fill(sortedJets[i].constituents()[j].pt());
            h_Electrons__fX -> Fill(event[EvIndex].xProd()/10.0);
            h_Electrons__fY -> Fill(event[EvIndex].yProd()/10.0);
            h_Electrons__fZ -> Fill(event[EvIndex].zProd()/10.0);
            h__pTQuotient_Electron_ak5FastJet -> Fill(sortedJets[i].constituents()[j].pt()/sortedJets[i].pt());
            if ( abs(event[EvIndex].phi() - sortedJets[i].phi_std()) <= PI ) {
              h__D__Electron_ak5FastJet -> Fill(sqrt( pow2(event[EvIndex].eta()-sortedJets[i].eta()) + pow2(event[EvIndex].phi()-sortedJets[i].phi_std()) ));
            } else {
              h__D__Electron_ak5FastJet -> Fill(sqrt( pow2(event[EvIndex].eta()-sortedJets[i].eta()) + pow2(2*PI-abs(event[EvIndex].phi()-sortedJets[i].phi_std())) ));
            }
