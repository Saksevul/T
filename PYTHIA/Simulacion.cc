...
int main(int argc, char* argv[]) {   Float_t PI=3.1415927;
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {    if (!pythia.next()) continue;
    ...
    for (size_t i=0; i<sortedJets.size(); i++){   Bool_t FlagE=false, FlagM=false; Float_t pTE=0.0, pTM=0.0; Int_t motherE, motherM;
      ...
      for (size_t j=0; j<sortedJets[i].constituents().size(); j++) {  Int_t EIndex=sortedJets[i].constituents()[j].user_info<MyInfo>().Index(); // Event Index.

        if ( abs(event[EIndex].id()) == 13 ) {  motherM=event[EIndex].mother1();
        ...
        // sortedJets[i].constituents()[j].user_info<MyInfo>().vertex_fY(), sortedJets[i].constituents()[j].user_info<MyInfo>().vertex_fZ() );
          if ( pow2(sortedJets[i].constituents()[j].user_info<MyInfo>().vertex_fX()) + pow2(sortedJets[i].constituents()[j].user_info<MyInfo>().vertex_fY()) + pow2(sortedJets[i].constituents()[j].user_info<MyInfo>().vertex_fZ()) <= 0.25 ) {
            ...
            if ( abs(event[EIndex].phi() - sortedJets[i].phi_std()) <= PI ) {
              h__D__Muon_ak5FastJet -> Fill(sqrt( pow2(event[EIndex].eta()-sortedJets[i].eta()) + pow2(event[EIndex].phi()-sortedJets[i].phi_std()) ));
            } else {
              h__D__Muon_ak5FastJet -> Fill(sqrt( pow2(event[EIndex].eta()-sortedJets[i].eta()) + pow2(2*PI-abs(event[EIndex].phi()-sortedJets[i].phi_std())) ));
            }
          ...
          // while ( abs(event[motherM].id()) >= 6 ) {
          //   motherM=event[motherM].mother1();
          // }
          // printf(" Madre final de este Muon = %i.\n", event[motherM].id());
        }

        if ( abs(event[EIndex].id()) == 11 ) {  motherE=event[EIndex].mother1();
          if ( pow2(event[EIndex].xProd()) + pow2(event[EIndex].yProd()) + pow2(event[EIndex].zProd()) <= 0.25 ) {
            h_Electrons__pT -> Fill(sortedJets[i].constituents()[j].pt());
            h_Electrons__fX -> Fill(event[EIndex].xProd()/10.0);
            h_Electrons__fY -> Fill(event[EIndex].yProd()/10.0);
            h_Electrons__fZ -> Fill(event[EIndex].zProd()/10.0);
            h__pTQuotient_Electron_ak5FastJet -> Fill(sortedJets[i].constituents()[j].pt()/sortedJets[i].pt());
            if ( abs(event[EIndex].phi() - sortedJets[i].phi_std()) <= PI ) {
              h__D__Electron_ak5FastJet -> Fill(sqrt( pow2(event[EIndex].eta()-sortedJets[i].eta()) + pow2(event[EIndex].phi()-sortedJets[i].phi_std()) ));
            } else {
              h__D__Electron_ak5FastJet -> Fill(sqrt( pow2(event[EIndex].eta()-sortedJets[i].eta()) + pow2(2*PI-abs(event[EIndex].phi()-sortedJets[i].phi_std())) ));
            }
            ...
            h_ak5FastJet__phi-> Fill(event[EIndex].phi());
          }
          // while ( abs(event[motherE].id()) >= 6 ) {
          //   motherE=event[motherE].mother1();
          // }
          // printf(" Madre final de este Electron = %i.\n", event[motherE].id());
        }
