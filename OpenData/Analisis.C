void Analisis() {   Float_t PI=TMath::Pi(); Int_t nprint=1;
  ...
  // TBranch *Electrons = Eventos->GetBranch("recoElectrons_Electrons__RECO.obj");
  ...
  TH1D *h_Muons_fX_   = new TH1D("Muons_fX_",   "Vertice en X de Muones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  TH1D *h_Muons_fY_   = new TH1D("Muons_fY_",   "Vertice en Y de Muones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  TH1D *h_Muons_fZ_   = new TH1D("Muons_fZ_",   "Vertice en Z de Muones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  // Electrons
  // TH1F *h_Electrons_pt_   = new TH1F("Electrons_pt_",   "p_{T} de Electrones contenidos en ak5PFJets; p_{T} [GeV]; Frecuencia",  900, 0, 450);
  // TH1F *h_Electrons_eta_  = new TH1F("Electrons_eta_",  "recoElectrons_Electrons__RECO.obj.eta_; #eta; Frecuencia", 59, -5.9, 5.9);
  TH1D *h_Electrons_fX_   = new TH1D("Electrons_fX_",   "Vertice en X de Electrones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  TH1D *h_Electrons_fY_   = new TH1D("Electrons_fY_",   "Vertice en Y de Electrones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  TH1D *h_Electrons_fZ_   = new TH1D("Electrons_fZ_",   "Vertice en Z de Electrones contenidos en ak5PFJets; Distancia [cm]; Frecuencia", 39, -1.95, 1.95);
  // // ak5PFJets
  ...
  TH1F *h__pTQuotient_Electron_ak5PFJet  = new TH1F("Cociente_pT__Electron-ak5PFJet",  "Cociente p_{T}  Electron / ak5PFJet; Indice; Frecuencia", 100, 0, 2);
  TH1F *h__pTQuotient_1Electron_ak5PFJet = new TH1F("Cociente_pT<100__1Electron-ak5PFJet",  "Cociente_{p_{T} < 100}  Electron_{MasEnergetico} / ak5PFJet; Indice; Frecuencia", 100, 0, 2);
  TH1F *h__pTQuot100__1Electron_ak5PFJet = new TH1F("Cociente_pT>100__1Electron-ak5PFJet",  "Cociente_{p_{T} > 100}  Electron_{MasEnergetico} / ak5PFJet; Indice; Frecuencia", 100, 0, 2);

  TH1F *h_ak5PFJets__EnergyQuotient_ChaHad_Jet  = new TH1F("Cociente_ak5PFJets__Energia_ChaHad-p_T_Jet", "Cociente Energia_{ChaHad}/Energia_{Jet} para cada ak5PFJet; Energia_{ChaHad}/Energia_{ak5PFJet}; Frecuencia", 20, 0, 2);
  TH1F *h_ak5PFJets__EnergyQuotient_NeuHad_Jet  = new TH1F("Cociente_ak5PFJets__Energia_NeuHad-p_T_Jet", "Cociente Energia_{NeuHad}/Energia_{Jet} para cada ak5PFJet; Energia_{NueHad}/Energia_{ak5PFJet}; Frecuencia", 20, 0, 2);
  TH1F *h_ak5PFJets__EnergyQuotient_Electron_Jet= new TH1F("Cociente_ak5PFJets__Energia_Electron-p_T_Jet", "Cociente Energia_{Electron}/Energia_{Jet} para cada ak5PFJet; Energia_{Electron}/Energia_{ak5PFJet}; Frecuencia", 20, 0, 2);
  TH1F *h_ak5PFJets__EnergyQuotient_Photon_Jet  = new TH1F("Cociente_ak5PFJets__Energia_Photon-p_T_Jet", "Cociente Energia_{Photon}/Energia_{Jet} para cada ak5PFJet; Energia_{Photon}/Energia_{ak5PFJet}; Frecuencia", 20, 0, 2);
  TH1F *h_ak5PFJets__EnergyQuotient_Muon_Jet    = new TH1F("Cociente_ak5PFJets__Energia_Muon-p_T_Jet", "Cociente Energia_{Muon}/Energia_{Jet} para cada ak5PFJet; Energia_{Muon}/Energia_{ak5PFJet}; Frecuencia", 20, 0, 2);
  // Distancia eta.
  ...
  TH1F *h__eta_D__Electron_ak5PFJet = new TH1F("D_eta__Electron-ak5PFJet", "Diferencia |#eta_{Electron} - #eta_{ak5PFJet}|; Distancia angular; Frecuencia", 120, 0, 1.2);
  // Distancia phi.
  ...
  TH1F *h__phi_D__Electron_ak5PFJet = new TH1F("D_phi__Electron-ak5PFJet", "Diferencia |#phi_{Electron} - #phi_{ak5PFJet}|; Distancia angular; Frecuencia", 120, 0, 1.2);
  // Distancia angular.
  ...
  TH1F *h__D__Electron_ak5PFJet = new TH1F("D__Electron-ak5PFJet", "Distancia angular #sqrt{(#Delta#phi)^{2} + (#Delta#eta)^{2}} del Electron al ak5PFJet; Valor; Frecuencia", 480, 0, 1.2);
  ...
  for (Int_t e=0; e<Eventos->GetEntries(); e++) {   // Int_t MultJet_04=0, MultJet_08=0, MultJet_16=0, MultJet_32=0, MultJet_64=0;
    ...
    Muons_pt_ ->GetBranch()->GetEntry(e);  Electrons_pt_ ->GetBranch()->GetEntry(e);  ak5PFJets_pt_  ->GetBranch()->GetEntry(e);
    Muons_eta_->GetBranch()->GetEntry(e);  Electrons_eta_->GetBranch()->GetEntry(e);  ak5PFJets_eta_ ->GetBranch()->GetEntry(e);
    Muons_phi_->GetBranch()->GetEntry(e);  Electrons_phi_->GetBranch()->GetEntry(e);  ak5PFJets_phi_ ->GetBranch()->GetEntry(e);
    ...
    for (Int_t i=0; i<ak5PFJets_pt_->GetLen(); i++) {      Float_t iPF_pT=ak5PFJets_pt_->GetValue(i), iPF_eta=ak5PFJets_eta_->GetValue(i), iPF_phi=ak5PFJets_phi_->GetValue(i);
      ...
      Float_t JetEnergy=ak5PFJets_mChargedHadronEnergy->GetValue(i)+ak5PFJets_mNeutralHadronEnergy->GetValue(i)+ak5PFJets_mElectronEnergy->GetValue(i)+ak5PFJets_mPhotonEnergy->GetValue(i)+ak5PFJets_mMuonEnergy->GetValue(i);
      ...
      if (ak5PFJets_mChargedHadronMultiplicity->GetValue(i) != 0) {
        h_ak5PFJets__EnergyQuotient_ChaHad_Jet -> Fill(ak5PFJets_mChargedHadronEnergy->GetValue(i)/JetEnergy);
      ...
      if (ak5PFJets_mNeutralHadronMultiplicity->GetValue(i) != 0) {
        h_ak5PFJets__EnergyQuotient_NeuHad_Jet -> Fill(ak5PFJets_mNeutralHadronEnergy->GetValue(i)/JetEnergy);
      ...
      if (ak5PFJets_mElectronMultiplicity->GetValue(i) != 0) {
        h_ak5PFJets__EnergyQuotient_Electron_Jet -> Fill(ak5PFJets_mElectronEnergy->GetValue(i)/JetEnergy);
      ...
      if (ak5PFJets_mPhotonMultiplicity->GetValue(i) != 0) {
        h_ak5PFJets__EnergyQuotient_Photon_Jet -> Fill(ak5PFJets_mPhotonEnergy->GetValue(i)/JetEnergy);
        h_ak5PFJets__PhotonMultiplicity -> Fill(ak5PFJets_mPhotonMultiplicity->GetValue(i));      }
      if (ak5PFJets_mMuonMultiplicity->GetValue(i) != 0) {
        h_ak5PFJets__EnergyQuotient_Muon_Jet -> Fill(ak5PFJets_mMuonEnergy->GetValue(i)/JetEnergy);
      ...
      if (ak5PFJets_mMuonMultiplicity->GetValue(i) > 0) {   Int_t Index[100]; Float_t D[100]; Bool_t Flag=false; Float_t pT=0.0; // Defino valores de referencia. Estos cambiarán más adelante.
        ...
        for (Int_t j=0; j<ak5PFJets_mMuonMultiplicity->GetValue(i); j++) {
          if ( Muons_pt_->GetValue(Index[j]) >= 5.5 && fabs(Muons_eta_->GetValue(Index[j])) < 1.48 ) {
        ...
        if ( iPF_pT <= 100.0 && Flag == true ) h__pTQuotient_1Muon_ak5PFJet -> Fill(pT/iPF_pT);              // Cociente_pT respecto al Muón más energético.
      ...
      if (ak5PFJets_mElectronMultiplicity->GetValue(i) > 0) {   Int_t Index[100]; Float_t D[100]; Bool_t Flag=false; Float_t pT=0.0;
        for (Int_t j=0; j<Electrons_pt_->GetLen(); j++) {
          if ( fabs(Electrons_phi_->GetValue(j)-iPF_phi) <= PI ) {
            D[j] = sqrt(pow(Electrons_eta_->GetValue(j)-iPF_eta,2)+pow(Electrons_phi_->GetValue(j)-iPF_phi,2));
          } else { D[j] = sqrt(pow(Electrons_eta_->GetValue(j)-iPF_eta,2)+pow(2*PI-fabs(Electrons_phi_->GetValue(j)-iPF_phi),2));  }
        }
        TMath::Sort(Electrons_pt_->GetLen(), D, Index, false);
        for (Int_t j=0; j<ak5PFJets_mElectronMultiplicity->GetValue(i); j++) {
          if ( Electrons_pt_->GetValue(Index[j]) >= 5.5 && fabs(Electrons_eta_->GetValue(Index[j])) < 1.48 ) {
            if ( pow(Electrons_fX_->GetValue(Index[j])-x_e,2) + pow(Electrons_fY_->GetValue(Index[j])-y_e,2) + pow(Electrons_fZ_->GetValue(Index[j])-z_e,2) <= 0.25 ) {
              ...
              h__pTQuotient_Electron_ak5PFJet -> Fill(Electrons_pt_->GetValue(Index[j])/iPF_pT);
              h__eta_D__Electron_ak5PFJet -> Fill(fabs(Electrons_eta_->GetValue(Index[j]) - ak5PFJets_eta_->GetValue(i)));
              h__phi_D__Electron_ak5PFJet -> Fill(fabs(Electrons_phi_->GetValue(Index[j]) - ak5PFJets_phi_->GetValue(i)));
              h__D__Electron_ak5PFJet -> Fill(D[Index[j]]);
              if (Electrons_pt_->GetValue(Index[j]) > pT) { Flag=true; pT=Electrons_pt_->GetValue(Index[j]); }
            }
          }
        }
        if ( iPF_pT <= 100.0 && Flag == true ) h__pTQuotient_1Electron_ak5PFJet -> Fill(pT/iPF_pT);
        else if (Flag == true) h__pTQuot100__1Electron_ak5PFJet -> Fill(pT/iPF_pT);
      }
    } // for loop i
  ...
  h__pTQuotient_Electron_ak5PFJet -> Write();
  h__pTQuotient_1Electron_ak5PFJet-> Write();
  h__pTQuot100__1Electron_ak5PFJet-> Write();
